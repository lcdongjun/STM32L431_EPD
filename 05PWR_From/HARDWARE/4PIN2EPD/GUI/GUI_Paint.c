...
{		    
	...
	switch(size)
	{
		...
		case 24:
			...
			break;
		case 48:
			W25QXX_Read(mat,foffset+ftinfo.f48addr,csize);
			break;
...
void Paint_Show_Font(u16 x, u16 y, u8 *font, u8 size, u8 mode, u8 cover) {
    ...
    u16 y0 = y;
    u8 dzk[72];
    u8 csize = (size * size + 7) / 8;  // 每个字符字节数
    if (size != 12 && size != 16 && size != 24) return;  // 不支持的size

    Paint_Get_HzMat(font, dzk, size);  // 获取相应大小的点阵数据
    for (t = 0; t < csize; t++) {
        temp = dzk[t];  // 得到点阵数据
        for (t1 = 0; t1 < 8; t1++) {
            if (mode == 0) {
                if (cover == 1) Paint_SetPixel(x, y, EPD_BLACK);  // 先填充背景色
                if (temp & 0x80) Paint_SetPixel(x, y, EPD_WHITE);  // 设置像素
            } else {
                if (cover == 1) Paint_SetPixel(x, y, EPD_WHITE);  // 先填充背景色
                if (temp & 0x80) Paint_SetPixel(x, y, EPD_BLACK);  // 设置像素
            }
            temp <<= 1;  // 左移1位，处理下一个像素
            y++;  // 向下移动一行
            // 如果到了字符的高度，换到下一列
