...
void W25QXX_Erase_By_Size(uint32_t Start_Addr, uint32_t Erase_Size)
{
    uint32_t end_addr = Start_Addr + Erase_Size;  // 计算结束地址
    uint32_t current_sector_start_addr;

    // 从起始地址开始，逐个扇区擦除
    for (current_sector_start_addr = Start_Addr; current_sector_start_addr < end_addr; current_sector_start_addr += 4096)
    {
        // 计算当前扇区的起始地址
        uint32_t sector_start_addr = (current_sector_start_addr / 4096) * 4096;

        // 擦除当前扇区
        W25QXX_Erase_Sector(sector_start_addr / 4096);
    }
}
