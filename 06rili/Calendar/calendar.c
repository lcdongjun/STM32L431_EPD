...
void initCalendar(u8 Calendar_x, u8 Calendar_y, u8 Time_x, u8 Time_y)
{
    ...
    HAL_RTC_GetDate(&hrtc, &RTC_DateStruct, RTC_FORMAT_BIN);
		...
		CalendarStruct.Time.Year = RTC_DateStruct.Year+2000;
		CalendarStruct.Time.Month = RTC_DateStruct.Month;
		CalendarStruct.Time.Day = RTC_DateStruct.Date;
		CalendarStruct.Time.Hour = RTC_TimeStruct.Hours;
		CalendarStruct.Time.Min = RTC_TimeStruct.Minutes;
		CalendarStruct.Time.Sec = RTC_TimeStruct.Seconds;
	
		new_date = RTC_DateStruct.Date;
		new_min = RTC_TimeStruct.Minutes;
		if(new_min!=old_min)
		{
			old_min = new_min;
			flage++;
		...
		{
			old_date = new_date;
			initYear(&CalendarStruct,RTC_DateStruct.Year+2000,RTC_DateStruct.Month,RTC_DateStruct.Date);
